...
Provides simple CSV logging with the columns:
  timestamp, water_level, pump_state, water_volume

Functions:
  init_log() -> ensures data directory & CSV header exist
  append_row(water_level, pump_state, water_volume, timestamp=None) -> append one row
...
def init_log() -> None:
    ...
    if need_header:
        with open(LOG_FILE, "w", newline="", encoding="utf-8") as fh:
            ...
            writer.writerow(["timestamp", "water_level", "pump_state", "water_volume"])


def append_row(water_level: float, pump_state: bool, water_volume: float, timestamp: Optional[datetime] = None) -> None:
    """Append a row to the CSV log.

	Args:
		water_level: water level value (usually percent 0..100) -- stored as float
		pump_state: True if pump is on, False otherwise
		water_volume: current water volume in the tank (e.g., liters)
    ...
    row = [timestamp.isoformat(), float(water_level), "on" if pump_state else "off", float(water_volume)]
